...
use embedded_graphics::{mono_font::{ascii, MonoTextStyle}, pixelcolor::Rgb565, prelude::*, text::{Alignment, Text}};
        ...
        match next_state {
            ScreenState::OnUntil{ hour, min } => {
                ...
                if let Ok(()) = write!(&mut buf, "on until {hour}:{min:02}") {
                    ...
                    let text = Text::with_alignment(buf.as_str(), display.bounding_box().center(), style, Alignment::Center);
